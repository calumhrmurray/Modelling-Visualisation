...
public class Stats{
	...
	// single bootstrap
	public static double sbootstrap(double[] sArray, int N, double T, final int W, double kb){
		// number of Cv,Chi taken from bootstrap
		int n = 10;
		double[] rArray = new double[N];
		double[] bootArray = new double[n];
		double error = 0;
		// calc. c n times using resampled data
		for (int i = 0; i<n; i++){
			// pick out n measurements at random and compute Cv, Chi
			for (int j = 0; j<N; j++){
				rArray[j] = sArray[(int)(Math.random()*N)];	
			}
			// array with n values of Cv and Chi
			//bootArray[i] = calc(rArray,N,T,W,kb)[0];
		}	
		// calculate sum of (ci-c)2
		error = variance(bootArray,n,N);
		error = Math.sqrt(sigArray);
		return error;
	}
